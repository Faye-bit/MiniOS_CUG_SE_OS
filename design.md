...
graph TB
    subgraph "用户空间"
        CLI["minos-client<br/>(CLI 命令行)"]
    end

    subgraph "服务端进程"
        SERVER["minos-server<br/>(Daemon 守护进程)"]
...
```mermaid
graph TD
    subgraph minos-client
        CLI_MAIN["main.rs<br/>clap 命令解析"]
    end

    subgraph minos-server
...
graph TD
    subgraph "Cargo Workspace"
        LIB["minos-lib<br/>(核心库)"]
        SERVER_BIN["minos-server<br/>(服务端二进制)"]
        CLIENT_BIN["minos-client<br/>(客户端二进制)"]
    end

    SERVER_BIN --> LIB
    CLIENT_BIN --> LIB

    subgraph "minos-lib 内部模块"
        COMMON_LIB["common"]
        STORAGE_LIB["storage ★"]
        SHM_LIB["shm ★"]
        CACHE_LIB["cache"]
